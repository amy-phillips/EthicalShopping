...
function colour_page(response) {
    ...
    // search results
    // and shopping basket
    $('.product-tile--title').each( function( index, product_div ){
    ...
    $('.product-details-tile__title').each( function( index, product_div ){
        if($(product_div).parent().find('#es-moar-infos').length==0) {
            colour_product(munged_tables, product_div, $(product_div).text().trim());
        }
    });
    ...
    $('.mini-tile__title-wrapper').each( function( index, product_div ){
        if($(product_div).parent().find('#es-moar-infos').length==0) {
            colour_product(munged_tables, product_div, $(product_div).text().trim());
        }
    });
